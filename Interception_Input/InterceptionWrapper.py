...
class InterceptionFilterKeyState(object):
    ...
    # fix: DOWN trebuie mapat la KEY_DOWN, nu la KEY_UP
    INTERCEPTION_FILTER_KEY_DOWN             = InterceptionKeyState.INTERCEPTION_KEY_DOWN
...
class InterceptionWrapper(object):
    ...
    def __init__(self):
        # Load DLL into memory (cu mesaj clar dacă nu poate fi încărcat)
        try:
            self.interceptionDll = ctypes.WinDLL("./wininputcore.dll")
        except OSError as e:
            raise RuntimeError(f"Cannot load wininputcore.dll from current folder: {e}")
        ...
        # fix: funcția nativă primește și milliseconds (unsigned long)
        self.interceptionDll.interception_wait_with_timeout.argtypes = [InterceptionContext_p, ctypes.c_ulong]
    ...
    def interception_wait_with_timeout(self, context, milliseconds):
        """
        InterceptionDevice ITERCEPTION_API interception_wait_with_timeout(InterceptionContext context, unsigned long milliseconds);
        """
        return self.interceptionDll.interception_wait_with_timeout(context, milliseconds)
