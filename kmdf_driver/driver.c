...
AionEvtDeviceAdd(_In_ WDFDRIVER Driver, _Inout_ PWDFDEVICE_INIT DeviceInit)
{
    ...
    // Initialize the VHF_CONFIG structure using the helper macro available
    // in recent WDK headers. This sets Size and sensible defaults for the
    // fields we don't explicitly set.
    ...
    if (!NT_SUCCESS(status)) {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            // We will continue, but input publishing may not occur.
        } else {
...
SendKeyboardReportToVhf(PDEVICE_CONTEXT DevCtx, UCHAR Modifier, UCHAR *KeyCodes, SIZE_T KeyCount)
{
    ...
    report[0] = Modifier;
    report[1] = 0; // reserved
    // Validate and copy keys defensively to avoid overruns and null-deref.
    if (KeyCount > 6) {
        KeyCount = 6;
    }
    ...
    // Build a HID_XFER_PACKET as expected by the VhfReadReportSubmit API
    HID_XFER_PACKET packet = { 0 };
    packet.reportBuffer = report;
    packet.reportBufferLen = sizeof(report);
    packet.reportId = 0; // our report descriptor uses report id 0
...
AionEvtDeviceContextCleanup(_In_ WDFOBJECT Object)
{
    ...
    if (devCtx && devCtx->VhfHandle) {
        ...
        // VhfStop is not present in newer WDK headers; delete is sufficient
        VhfDelete(devCtx->VhfHandle, TRUE);
                      ...
                      _In_ size_t InputBufferLength, _In_ ULONG IoControlCode)
{
    ...
    case IOCTL_HID_KEYUP:
        ...
        if (NT_SUCCESS(status)) {
                ...
                // Reference variable explicitly so static analysis sees it is used
